...
const newDepositoryMenu = require('./newDepositoryMenu');
const entityDepositoryMenu = require('./entityDepositoryMenu');

const depositoryMenu = (options) => {

  let menu = [
  { type: 'list', name: 'action', message: "Welcome to the Depositories menu. What would you like to do?", choices: depositoryChoices }
  ]
  ...
  inquirer
    .prompt(menu).then(submenu => {
      ...
      case 'POST':
        options.method = submenu.action;
        newDepositoryMenu.newDepositoryMenu(options);
        break;
      case 'more':
        options.method = 'GET';
...
module.exports.depositoryMenu = depositoryMenu;
